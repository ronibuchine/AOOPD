...
public class AgglomerativeClustering<T extends Clusterable<T>> implements Clustering<T> {
	...
	public Set<Set<T>> clusterSet(Set<T> elements) {
		...
		// initialize clusters
		Set<Set<T>> clusters = elements.stream().map(elem -> new HashSet<>(Arrays.asList(elem)))
				.collect(Collectors.toSet());

		while (clusters.size() > 1) {

			// find the 2 closest clusters (c1 and c2)
			Optional<SimpleEntry<Set<T>, Set<T>>> closestClusters = clusters.stream()
					...
					.min(Comparator.comparingDouble(
							clusterPair -> distanceBetweenSets(clusterPair.getKey(), clusterPair.getValue())));
